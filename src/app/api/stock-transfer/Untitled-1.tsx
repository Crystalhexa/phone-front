...
async function executeCompleteTransfer(
  client: PoolClient,
  sendBranchId: string,
  ...
  for (const product of products) {
    for (const batch of product.batches) {
      // Get batch details for the transfer
      const batchResult = await client.query(`
      ...
      // 1. Remove from source branch inventory
      await client.query(`
      ...
      // 3. Check if destination branch has inventory record for this product
      const destInventoryResult = await client.query(`
        SELECT id FROM branch_inventory 
        WHERE branch_id = $1 AND product_id = $2
      `, [receiveBranchId, product.product_id])
      ...
      if (destInventoryResult.rows.length === 0) {
        ...
        const newInventoryResult = await client.query(`
          ...
          RETURNING id
        `, [receiveBranchId, product.product_id, batch.count])
        
        destInventoryId = newInventoryResult.rows[0].id
      } else {
        // Update existing inventory record
        destInventoryId = destInventoryResult.rows[0].id
        ...
        await client.query(`
          ...
          WHERE id = $2
        `, [batch.count, destInventoryId])
      }
      
      // 4. Check if destination branch already has this batch
      ...
      `, [destInventoryId, batch.batch_id])
      ...
      if (existingBatchResult.rows.length === 0) {
        ...
        await client.query(`
          ...
          SET quantity = quantity + $1
          WHERE id = $2
        `, [batch.count, existingBatchResult.rows[0].id])
      }
    }
  }
}

// Remove the reserveInventory function as we're doing direct transfer
// async function reserveInventory(...) - REMOVED
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      // 5. Create transfer items (with all quantities filled)
      await createTransferItems(client, transferRequestId, body.products)
      
      // 6. Execute complete inventory transfer
      await executeCompleteTransfer(
        client,
        body.send_branch_id,
      ...
      // 8. Create transfer completion log
    ...
    return NextResponse.json<ApiResponse>({
      ...
      timestamp: new Date().toISOString()
    }, { status: 200 }) // Changed from 201 to 200 since it's completed
