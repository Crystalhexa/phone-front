...
const ProductForm: React.FC = () => {
  ...
  const validateNewBarcode = () => {
    try {
      NewBarcodeSchema.parse(newBarcode);
      setBarcodeErrors({});
      return true;
    } catch (error) {
      if (error instanceof z.ZodError) {
  ...
  const removeBarcode = (id: string) => {
    setFormData(prev => ({
      ...prev,
      barcodes: prev.barcodes.filter(bar => bar.id !== id),
    }));
  };
  ...
  const regenerateAutoFields = () => {
    ...
    if (brand && subcategory) {
      ...
      // Generate SKU
      let sku = `${brand.name.replace(/\s+/g, '')}-${subcategory.name.replace(/\s+/g, '')}`;
      if (formData.model) {
        sku += `-${formData.model.replace(/\s+/g, '')}`;
      }
      formData.specifications.forEach(spec => {
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4" data-spec-section>
                  <div className="space-y-1">
                    <Input
                      ...
                      onChange={(e) => setNewBarcode(prev => ({ ...prev, code: e.target.value }))}
                      className={`${barcodeErrors.code ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'} bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100`}
                    />
                    <ErrorMessage message={barcodeErrors.code} />
                  </div>
                  <Button
                    type="button"
                    onClick={addBarcode}
                ...
                {formData.barcodes.map(bar => (
                  <Badge
                    ...
                    className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-1"
                  >
                    <span>{bar.code}</span> {/* Remove the colon since it's just the barcode */}
                    <button
                      type="button"
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center space-x-3">
                ...
                <Badge variant={formData.is_unique ? "default" : "secondary"} className="ml-2">
