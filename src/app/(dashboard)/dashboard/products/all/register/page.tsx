...
export default function VariableProductForm() {
  ...
  const [resetTrigger, setResetTrigger] = useState(0); // Add reset trigger for subcomponents
  ...
  const handleGenerateVariations = () => {
    ...
    if (attributeEntries.length === 0) {
      toast.error("Please select at least one attribute to generate variations.");
      return;
    }
    ...
    toast.success(`Successfully generated ${newVariations.length} variations.`);
  };
  ...
  const resetFormState = () => {
    ...
    // Trigger reset in subcomponents
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    if (!isVariable && !data.barcode) {
      ...
      toast.error("Barcode is required for non-variable products.");
      return;
    }

    if (isVariable && (!data.variations || data.variations.length === 0)) {
      setSubmitStatus('error');
      toast.error("At least one variation is required for variable products.");
      return;
    }
    ...
    if (isVariable && data.variations) {
      ...
      if (invalidVariations.length > 0) {
        setSubmitStatus('error');
        toast.error("All variations must have name, barcode, SKU, and attribute combinations.")
        return;
      }
    }
    ...
    try {
      ...
      // Show loading toast
      toast.success("Please wait while we save your product...");
      ...
      const payload = {
        ...
        default_variation: {
          ...
          warranty_period: data.warrantyPeriod || 0,
          barcode: data.barcode, // Required for non-variable products
        }
      };
