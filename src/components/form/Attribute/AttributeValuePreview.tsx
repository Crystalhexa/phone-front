...
import { AttributePreviewProps } from '@/types/form';

export const AttributeValuePreview: React.FC<AttributePreviewProps> = ({
  values,
  onRemove,
  onReorder,
    ...
    <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-md border">
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
          Preview ({values.length} subcategories):
        </p>
        {values.length > 0 && (
          <button
            type="button"
            onClick={() => values.forEach((_, index) => onRemove(index))}
      ...
      {enableDragDrop && onReorder ? (
                ...
                {values.map((value, index) => (
                  ...
                  >
                    {(provided, snapshot) => (
                      ...
                      >
                        <SubcategoryTag subcategory={value} index={index} />
                      </div>
                    )}
                  </Draggable>
                ))}
        ...
        <div className="flex flex-wrap gap-2">
          {values.map((value, index) => (
            <SubcategoryTag key={`${value}-${index}`} subcategory={value} index={index} />
          ))}
        </div>
      )}
    </div>
