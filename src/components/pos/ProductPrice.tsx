...
export const ProductPrice: React.FC<{
  ...
  onDeleteBarcode?: (cartIndex: number, barcodeId: string,barcode:string) => void; // New prop for barcode deletion
  onDeleteEntireItem?: (cartIndex: number) => void; // New prop for entire item deletion when no barcodes left
}> = ({
  product,
  index,
  onUpdateDiscount,
  ...
  onDeleteEntireItem
    ...
    const [itemToDelete, setItemToDelete] = useState<string | null>(null) // Track which barcode is being deleted
    ...
    const handleDeleteBarcode = (barcodeId: string,barcode:string) => {
      ...
      const remainingBarcodes = product.item_barcodes?.filter(b => b.barcode_id !== barcodeId) || []

      if (remainingBarcodes.length === 0) {
        // If no barcodes left, delete entire item
        if (onDeleteEntireItem) {
          onDeleteEntireItem(index)
          toast.success('Item removed from cart - no barcodes remaining')
        } else {
          toast.error('Cannot delete last barcode without item removal handler')
        }
      } else {
        // Delete just this barcode
        onDeleteBarcode(index, barcodeId,barcode)
        toast.success('Barcode removed from cart')
      }
    ...
    const canDelete = onDeleteBarcode && product.type === 'INDIVIDUAL' && product.item_barcodes && product.item_barcodes.length > 0
        ...
        <DialogContent className="w-full max-w-2xl sm:max-w-3xl rounded-2xl p-0">
          <div className="max-h-[85vh] overflow-y-auto px-6 py-8">
            ...
            <div className="space-y-6">
              ...
              {/* Enhanced Individual Items Section with Delete Options */}
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center justify-between">
                      ...
                      {canDelete && product.item_barcodes.length === 1 && (
                        <div className="flex items-center gap-2 text-amber-600">
                          <AlertCircle className="w-4 h-4" />
                          <span className="text-xs">Last item - deletion will remove entire cart item</span>
                        </div>
                      )}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ScrollArea className="h-64 w-full overflow-y-auto">
                      <div className="space-y-2 pr-4  max-h-64">
                        {product.item_barcodes.map((item, idx) => (
                          <div
                            key={item.barcode_id || idx}
                            className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded transition-colors hover:bg-gray-100 dark:hover:bg-gray-700 group"
                          >
                            <div className="flex-1 min-w-0">
                              <p className="font-mono text-sm truncate font-medium">{item.barcode}</p>
                            ...
                            <div className="flex items-center gap-2 ml-3 shrink-0">
                              ...
                              {canDelete && (
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className="p-1 h-7 w-7 text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-opacity"
                                  onClick={() => handleDeleteBarcode(item.barcode_id,item.barcode)}
                                >
                    ...
                    {/* Delete functionality notice */}
                    {canDelete && (
                      <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-xs text-blue-700 dark:text-blue-300">
                        <Info className="w-3 h-3 inline mr-1" />
                        Hover over items to reveal delete options
                      </div>
                    )}
                  </CardContent>
                </Card>
              ...
              {/* Batch Section remains unchanged */}
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="space-y-2 text-xs text-muted-foreground">
                      ...
                      {canDelete && product.item_barcodes && (
                        <p>• {product.item_barcodes.length} barcode{product.item_barcodes.length !== 1 ? 's' : ''} available for management</p>
                      )}
                    </div>
                  </div>
                </CardContent>
